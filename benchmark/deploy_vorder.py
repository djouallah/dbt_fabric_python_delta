"""Deploy the benchmark semantic model(s) under benchmark/ (e.g. aemo_electricity_vorder) and
refresh them — used by the XMLA benchmark workflow so the vorder model is deployed by the
EXPERIMENT, NOT the demo's deploy.py (which stays a single, pristine model in fabric_items/).

Mirrors deploy.py's semantic-model deploy: GUID-swap the bim (dev -> target ws/lh), fab deploy
scoped to ./benchmark, then refresh via the Power BI API. Assumes `fab` is installed and logged
in (federated) by the workflow. Args: --env (deploy_config.yml section, default main).
"""
import argparse
import json
import re
...
allc = yaml.safe_load((root / "deploy_config.yml").read_text())
...
if not names:
    raise SystemExit("No *.SemanticModel found under benchmark/")
...
cfg_yml.write_text(
    f'core:\n  workspace: "{ws}"\n  repository_directory: "./benchmark"\n'
    '  item_types_in_scope:\n    - SemanticModel\n')
try:
